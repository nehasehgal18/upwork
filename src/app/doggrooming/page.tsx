...
export default function DogGroomingBusinessCalculator() {
  const [averageGroomingPrice, setAverageGroomingPrice] = useState<number>(50);
  const [groomsPerDay, setGroomsPerDay] = useState<number>(10);
  const [daysOpenPerMonth, setDaysOpenPerMonth] = useState<number>(26);
  const [suppliesCostPercentage, setSuppliesCostPercentage] = useState<number>(15);
  const [rentPerMonth, setRentPerMonth] = useState<number>(2000);
  const [staffCostPerMonth, setStaffCostPerMonth] = useState<number>(4000);
  ...
  const calculateFinancials = () => {
    const revenue = averageGroomingPrice * groomsPerDay * daysOpenPerMonth;
    const costOfGoods = revenue * (suppliesCostPercentage / 100);
    const operatingExpenses = rentPerMonth + staffCostPerMonth + utilitiesPerMonth + marketingCostPerMonth;
  ...
  useEffect(() => {
    calculateFinancials();
  }, [averageGroomingPrice, groomsPerDay, daysOpenPerMonth, suppliesCostPercentage, rentPerMonth, staffCostPerMonth, utilitiesPerMonth, marketingCostPerMonth]);
  ...
  return (
    <div className="max-w-4xl mx-auto mt-10 p-6 bg-white rounded-lg shadow-xl">
      <h1 className="text-3xl font-bold mb-6 text-center">Dog Grooming Business Calculator</h1>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <label htmlFor="averageGroomingPrice" className="block mb-2 font-medium">Average Grooming Price: {formatCurrency(averageGroomingPrice)}</label>
          <input
            type="range"
            id="averageGroomingPrice"
            value={averageGroomingPrice}
            onChange={(e) => setAverageGroomingPrice(Number(e.target.value))}
            className="w-full"
            min="20"
            max="150"
            step="5"
        ...
        <div>
          <label htmlFor="groomsPerDay" className="block mb-2 font-medium">Grooms per Day: {groomsPerDay}</label>
          <input
            type="range"
            id="groomsPerDay"
            value={groomsPerDay}
            onChange={(e) => setGroomsPerDay(Number(e.target.value))}
            className="w-full"
            min="5"
            max="30"
            step="1"
        ...
        <div>
          <label htmlFor="suppliesCostPercentage" className="block mb-2 font-medium">Supplies Cost Percentage: {suppliesCostPercentage}%</label>
          <input
            type="range"
            id="suppliesCostPercentage"
            value={suppliesCostPercentage}
            onChange={(e) => setSuppliesCostPercentage(Number(e.target.value))}
            className="w-full"
            min="5"
            max="25"
            step="1"
      ...
      <div className="mt-8 p-4 bg-gray-100 rounded-md grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <h2 className="text-xl font-semibold mb-2">Monthly Supplies Cost:</h2>
